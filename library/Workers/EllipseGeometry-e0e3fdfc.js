...
define(['exports', './when-7ef6387a', './DeveloperError-101d8eb9', './Check-71521db1', './GeometryOffsetAttribute-19130c81', './Math-4ee1dfaf', './Ellipsoid-e5098033', './Transforms-0f2c35ae', './Cartesian2-ad22f1aa', './ComponentDatatype-f4fabeaa', './GeometryAttribute-cb1e0b8f', './PrimitiveType-4c1d698a', './GeometryAttributes-cb18da36', './VertexFormat-64d8e6c6', './EllipseGeometryLibrary-65dcd445', './GeometryInstance-e03db032', './GeometryPipeline-6475d5b1', './IndexDatatype-ff78eee3'], function (exports, when, DeveloperError, Check, GeometryOffsetAttribute, _Math, Ellipsoid, Transforms, Cartesian2, ComponentDatatype, GeometryAttribute, PrimitiveType, GeometryAttributes, VertexFormat, EllipseGeometryLibrary, GeometryInstance, GeometryPipeline, IndexDatatype) { 'use strict';
    ...
    var scratchCartesian1 = new Ellipsoid.Cartesian3();
        ...
        var scratchNormal = new Ellipsoid.Cartesian3();
        var scratchTangent = new Ellipsoid.Cartesian3();
        var scratchBitangent = new Ellipsoid.Cartesian3();

        var scratchCartographic = new Ellipsoid.Cartographic();
        var projectedCenterScratch = new Ellipsoid.Cartesian3();
        ...
        function computeTopBottomAttributes(positions, options, extrude) {
            ...
            for (var i = 0; i < length; i += 3) {
                var i1 = i + 1;
                var i2 = i + 2;
                var position = Ellipsoid.Cartesian3.fromArray(positions, i, scratchCartesian1);
                ...
                if (vertexFormat.st) {
                    ...
                    Ellipsoid.Cartesian3.subtract(projectedPoint, projectedCenter, projectedPoint);
                ...
                if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent || shadowVolume) {
                    ...
                    if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                        if (vertexFormat.tangent || vertexFormat.bitangent) {
                            tangent = Ellipsoid.Cartesian3.normalize(Ellipsoid.Cartesian3.cross(Ellipsoid.Cartesian3.UNIT_Z, normal, tangent), tangent);
                        ...
                        if (vertexFormat.bitangent) {
                            bitangent = Ellipsoid.Cartesian3.normalize(Ellipsoid.Cartesian3.cross(normal, tangent, bitangent), bitangent);
        ...
        var boundingSphereCenter = new Ellipsoid.Cartesian3();

        function computeEllipse(options) {
            var center = options.center;
            boundingSphereCenter = Ellipsoid.Cartesian3.multiplyByScalar(options.ellipsoid.geodeticSurfaceNormal(center, boundingSphereCenter), options.height, boundingSphereCenter);
            boundingSphereCenter = Ellipsoid.Cartesian3.add(center, boundingSphereCenter, boundingSphereCenter);
        ...
        function computeWallAttributes(positions, options) {
            ...
            for (var i = 0; i < length; i += 3) {
                ...
                extrudedPosition = Ellipsoid.Cartesian3.clone(position, scratchCartesian2);
                normal = ellipsoid.geodeticSurfaceNormal(position, normal);
                ...
                var scaledNormal = Ellipsoid.Cartesian3.multiplyByScalar(normal, height, scratchCartesian4);
                position = Ellipsoid.Cartesian3.add(position, scaledNormal, position);
                scaledNormal = Ellipsoid.Cartesian3.multiplyByScalar(normal, extrudedHeight, scaledNormal);
                extrudedPosition = Ellipsoid.Cartesian3.add(extrudedPosition, scaledNormal, extrudedPosition);
                ...
                if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                    ...
                    bitangent = Ellipsoid.Cartesian3.clone(normal, bitangent);
                    var next = Ellipsoid.Cartesian3.fromArray(positions, (i + 3) % length, scratchCartesian4);
                    Ellipsoid.Cartesian3.subtract(next, position, next);
                    var bottom = Ellipsoid.Cartesian3.subtract(extrudedPosition, position, scratchCartesian3);

                    normal = Ellipsoid.Cartesian3.normalize(Ellipsoid.Cartesian3.cross(bottom, next, normal), normal);
                    ...
                    if (vertexFormat.tangent) {
                        tangent = Ellipsoid.Cartesian3.normalize(Ellipsoid.Cartesian3.cross(bitangent, normal, tangent), tangent);
                        tangents[i] = tangent.x;
                        tangents[i1] = tangent.y;
                        tangents[i2] = tangent.z;
        ...
        function computeExtrudedEllipse(options) {
            ...
            var ellipsoid = options.ellipsoid;
            var semiMajorAxis = options.semiMajorAxis;
            var scaledNormal = Ellipsoid.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scratchCartesian1), options.height, scratchCartesian1);
            topBoundingSphere.center = Ellipsoid.Cartesian3.add(center, scaledNormal, topBoundingSphere.center);
            topBoundingSphere.radius = semiMajorAxis;

            scaledNormal = Ellipsoid.Cartesian3.multiplyByScalar(ellipsoid.geodeticSurfaceNormal(center, scaledNormal), options.extrudedHeight, scaledNormal);
            bottomBoundingSphere.center = Ellipsoid.Cartesian3.add(center, scaledNormal, bottomBoundingSphere.center);
        ...
        function computeRectangle(center, semiMajorAxis, semiMinorAxis, rotation, granularity, ellipsoid, result) {
            ...
            for (var i = 0; i < positionsCount; ++i) {
                positions[i] = Ellipsoid.Cartesian3.fromArray(positionsFlat, i * 3);
            }
        ...
        function EllipseGeometry(options) {
            ...
            var center = options.center;
            var ellipsoid = when.defaultValue(options.ellipsoid, Ellipsoid.Ellipsoid.WGS84);
            var semiMajorAxis = options.semiMajorAxis;
            var semiMinorAxis = options.semiMinorAxis;
            ...
            if (semiMajorAxis < semiMinorAxis) {
                throw new DeveloperError.DeveloperError('semiMajorAxis must be greater than or equal to the semiMinorAxis.');
            }
            if (granularity <= 0.0) {
                throw new DeveloperError.DeveloperError('granularity must be greater than zero.');
            }
            //>>includeEnd('debug');
            ...
            this._center = Ellipsoid.Cartesian3.clone(center);
            this._semiMajorAxis = semiMajorAxis;
            this._semiMinorAxis = semiMinorAxis;
            this._ellipsoid = Ellipsoid.Ellipsoid.clone(ellipsoid);
         ...
         * @type {Number}
         */
        EllipseGeometry.packedLength = Ellipsoid.Cartesian3.packedLength + Ellipsoid.Ellipsoid.packedLength + VertexFormat.VertexFormat.packedLength + 9;
        ...
        EllipseGeometry.pack = function(value, array, startingIndex) {
            ...
            Ellipsoid.Cartesian3.pack(value._center, array, startingIndex);
        ...
        EllipseGeometry.unpack = function(array, startingIndex, result) {
            ...
            var center = Ellipsoid.Cartesian3.unpack(array, startingIndex, scratchCenter);
            startingIndex += Ellipsoid.Cartesian3.packedLength;
            ...
            result._center = Ellipsoid.Cartesian3.clone(center, result._center);
...
//# sourceMappingURL=EllipseGeometry-e0e3fdfc.js.map
