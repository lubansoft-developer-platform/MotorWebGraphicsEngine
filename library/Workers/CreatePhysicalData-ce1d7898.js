...
define(['exports', './when-7ef6387a', './DeveloperError-101d8eb9', './Math-4ee1dfaf', './Ellipsoid-e5098033', './Transforms-0f2c35ae', './IndexDatatype-ff78eee3'], function (exports, when, DeveloperError, _Math, Ellipsoid, Transforms, IndexDatatype) { 'use strict';
    ...
    /**
        ...
        function TerrainProvider() {
            DeveloperError.DeveloperError.throwInstantiationError();
        }
        ...
        Object.defineProperties(TerrainProvider.prototype, {
             ...
             */
            errorEvent : {
                get : DeveloperError.DeveloperError.throwInstantiationError
            },
        ...
        TerrainProvider.getRegularGridIndices = function(width, height) {
            ...
            if (width * height >= _Math.CesiumMath.FOUR_GIGABYTES) {
                throw new DeveloperError.DeveloperError('The total number of vertices (width * height) must be less than 4,294,967,296.');
            }
            //>>includeEnd('debug');
         ...
         */
        TerrainProvider.prototype.requestTileGeometry = DeveloperError.DeveloperError.throwInstantiationError;
         ...
         */
        TerrainProvider.prototype.getLevelMaximumGeometricError = DeveloperError.DeveloperError.throwInstantiationError;
         ...
         */
        TerrainProvider.prototype.getTileDataAvailable = DeveloperError.DeveloperError.throwInstantiationError;
         ...
         */
        TerrainProvider.prototype.loadTileDataAvailability = DeveloperError.DeveloperError.throwInstantiationError;

    class CreatePhysicalData {
    }

    var negateRelativeToCenter = new Ellipsoid.Cartesian3();
    var negateRelativeToCenterMatrix = new Transforms.Matrix4();
    var relativePosition = new Ellipsoid.Cartesian3();

    CreatePhysicalData.createPhysicalArray = function (physical, lbSpaMgr, relativeToCenter, positions, indices) {
        var hasRelativeToCenter = when.defined(relativeToCenter);
        relativeToCenter = hasRelativeToCenter ? relativeToCenter : Ellipsoid.Cartesian3.ZERO;
        Ellipsoid.Cartesian3.negate(relativeToCenter, negateRelativeToCenter);
        Transforms.Matrix4.fromTranslation(negateRelativeToCenter, negateRelativeToCenterMatrix);

        var physicalVertices = new Float32Array(positions.length * 3.0);
        var indexVertices = 0;
        for (var index = 0; index < positions.length; ++index) {
            var position = positions[index];
            Transforms.Matrix4.multiplyByPoint(negateRelativeToCenterMatrix, position, relativePosition);
        ...
        initSpaTrianglePrimitive(pPrimitive, physicalVertices, indices);
    ...
    function initSpaTrianglePrimitive(pPrimitive, pPtAry, pIndexAry) {
        pPrimitive.SetPtValNum(pPtAry.length, false);//true desc have batch
        var i;
        for (i = 0; i < pPtAry.length; ++i) {
            pPrimitive.SetPtValVal(i, pPtAry[i]);
        }
        pPrimitive.SetIndexNum(pIndexAry.length, false);//need edge
        for (i = 0; i < pIndexAry.length; ++i) {
            pPrimitive.SetIndexVal(i, pIndexAry[i]);
        }
    }

    exports.CreatePhysicalData = CreatePhysicalData;
    exports.TerrainProvider = TerrainProvider;

});
//# sourceMappingURL=CreatePhysicalData-ce1d7898.js.map
