...
define(['exports', './when-7ef6387a', './DeveloperError-101d8eb9', './GeometryOffsetAttribute-19130c81', './Math-4ee1dfaf', './Ellipsoid-e5098033', './Transforms-0f2c35ae', './Cartesian2-ad22f1aa', './ComponentDatatype-f4fabeaa', './GeometryAttribute-cb1e0b8f', './PrimitiveType-4c1d698a', './GeometryAttributes-cb18da36', './VertexFormat-64d8e6c6', './IndexDatatype-ff78eee3'], function (exports, when, DeveloperError, GeometryOffsetAttribute, _Math, Ellipsoid, Transforms, Cartesian2, ComponentDatatype, GeometryAttribute, PrimitiveType, GeometryAttributes, VertexFormat, IndexDatatype) { 'use strict';

    var scratchPosition = new Ellipsoid.Cartesian3();
        ...
        var scratchNormalST = new Ellipsoid.Cartesian3();
        var defaultRadii = new Ellipsoid.Cartesian3(1.0, 1.0, 1.0);
        ...
        function EllipsoidGeometry(options) {
            ...
            if (slicePartitions < 3) {
                throw new DeveloperError.DeveloperError('options.slicePartitions cannot be less than three.');
            }
            if (stackPartitions < 3) {
                throw new DeveloperError.DeveloperError('options.stackPartitions cannot be less than three.');
            }
            //>>includeEnd('debug');

            this._radii = Ellipsoid.Cartesian3.clone(radii);
            this._innerRadii = Ellipsoid.Cartesian3.clone(innerRadii);
         ...
         * @type {Number}
         */
        EllipsoidGeometry.packedLength = 2 * (Ellipsoid.Cartesian3.packedLength) + VertexFormat.VertexFormat.packedLength + 7;
        ...
        EllipsoidGeometry.pack = function(value, array, startingIndex) {
            ...
            if (!when.defined(value)) {
                throw new DeveloperError.DeveloperError('value is required');
            }
            if (!when.defined(array)) {
                throw new DeveloperError.DeveloperError('array is required');
            }
            //>>includeEnd('debug');

            startingIndex = when.defaultValue(startingIndex, 0);

            Ellipsoid.Cartesian3.pack(value._radii, array, startingIndex);
            startingIndex += Ellipsoid.Cartesian3.packedLength;

            Ellipsoid.Cartesian3.pack(value._innerRadii, array, startingIndex);
            startingIndex += Ellipsoid.Cartesian3.packedLength;
        ...
        var scratchRadii = new Ellipsoid.Cartesian3();
        var scratchInnerRadii = new Ellipsoid.Cartesian3();
        ...
        EllipsoidGeometry.unpack = function(array, startingIndex, result) {
            ...
            var radii = Ellipsoid.Cartesian3.unpack(array, startingIndex, scratchRadii);
            startingIndex += Ellipsoid.Cartesian3.packedLength;

            var innerRadii = Ellipsoid.Cartesian3.unpack(array, startingIndex, scratchInnerRadii);
            startingIndex += Ellipsoid.Cartesian3.packedLength;
            ...
            result._radii = Ellipsoid.Cartesian3.clone(radii, result._radii);
            result._innerRadii = Ellipsoid.Cartesian3.clone(innerRadii, result._innerRadii);
        ...
        EllipsoidGeometry.createGeometry = function(ellipsoidGeometry) {
            ...
            var ellipsoid;
            var ellipsoidOuter = Ellipsoid.Ellipsoid.fromCartesian3(radii);
            var ellipsoidInner = Ellipsoid.Ellipsoid.fromCartesian3(innerRadii);

            if (vertexFormat.st || vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
                for (i = 0; i < vertexCount; i++) {
                    ellipsoid = (isInner[i]) ? ellipsoidInner : ellipsoidOuter;
                    var position = Ellipsoid.Cartesian3.fromArray(positions, i * 3, scratchPosition);
                    ...
                    if (vertexFormat.tangent || vertexFormat.bitangent) {
                        ...
                        if ((!isTopOpen && (i >= tangetOffset && i < (tangetOffset + numThetas * 2)))) {
                            unit = Ellipsoid.Cartesian3.UNIT_X;
                        } else {
                            unit = Ellipsoid.Cartesian3.UNIT_Z;
                        }
                        Ellipsoid.Cartesian3.cross(unit, normal, tangent);
                        Ellipsoid.Cartesian3.normalize(tangent, tangent);
                        ...
                        if (vertexFormat.bitangent) {
                            var bitangent = Ellipsoid.Cartesian3.cross(normal, tangent, scratchBitangent);
                            Ellipsoid.Cartesian3.normalize(bitangent, bitangent);
        ...
        EllipsoidGeometry.getUnitEllipsoid = function() {
            if (!when.defined(unitEllipsoidGeometry)) {
                unitEllipsoidGeometry = EllipsoidGeometry.createGeometry((new EllipsoidGeometry({
                    radii : new Ellipsoid.Cartesian3(1.0, 1.0, 1.0),
...
//# sourceMappingURL=EllipsoidGeometry-f9dab503.js.map
