...
define(['exports', './when-7ef6387a', './DeveloperError-101d8eb9', './Check-71521db1', './Math-4ee1dfaf', './Ellipsoid-e5098033'], function (exports, when, DeveloperError, Check, _Math, Ellipsoid) { 'use strict';
    ...
    function calculateM(ellipticity, major, latitude) {
        ...
        var scratchCart1 = new Ellipsoid.Cartesian3();
        var scratchCart2 = new Ellipsoid.Cartesian3();

        function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
            var firstCartesian = Ellipsoid.Cartesian3.normalize(ellipsoid.cartographicToCartesian(start, scratchCart2), scratchCart1);
            var lastCartesian = Ellipsoid.Cartesian3.normalize(ellipsoid.cartographicToCartesian(end, scratchCart2), scratchCart2);

            //>>includeStart('debug', pragmas.debug);
            Check.Check.typeOf.number.greaterThanOrEquals('value', Math.abs(Math.abs(Ellipsoid.Cartesian3.angleBetween(firstCartesian, lastCartesian)) - Math.PI), 0.0125);
            //>>includeEnd('debug');
            ...
            ellipsoidRhumbLine._start = Ellipsoid.Cartographic.clone(start, ellipsoidRhumbLine._start);
            ellipsoidRhumbLine._start.height = 0;

            ellipsoidRhumbLine._end = Ellipsoid.Cartographic.clone(end, ellipsoidRhumbLine._end);
        ...
        function interpolateUsingSurfaceDistance(start, heading, distance, major, ellipticity, result)
        {
            ...
            return new Ellipsoid.Cartographic(longitude, latitude, 0);
        }
        ...
        function EllipsoidRhumbLine(start, end, ellipsoid) {
            var e = when.defaultValue(ellipsoid, Ellipsoid.Ellipsoid.WGS84);
            this._ellipsoid = e;
            this._start = new Ellipsoid.Cartographic();
            this._end = new Ellipsoid.Cartographic();
        ...
        EllipsoidRhumbLine.prototype.interpolateUsingSurfaceDistance = function(distance, result) {
            ...
            if (!when.defined(this._distance) || this._distance === 0.0) {
                throw new DeveloperError.DeveloperError('EllipsoidRhumbLine must have distinct start and end set.');
            }
            //>>includeEnd('debug');
        ...
        EllipsoidRhumbLine.prototype.findIntersectionWithLongitude = function(intersectionLongitude, result) {
            ...
            if (!when.defined(result)) {
                result = new Ellipsoid.Cartographic();
            }
        ...
        EllipsoidRhumbLine.prototype.findIntersectionWithLatitude = function(intersectionLatitude, result) {
            ...
            return new Ellipsoid.Cartographic(longitude, intersectionLatitude, 0);
        };

    exports.EllipsoidRhumbLine = EllipsoidRhumbLine;

});
//# sourceMappingURL=EllipsoidRhumbLine-89ea2621.js.map
