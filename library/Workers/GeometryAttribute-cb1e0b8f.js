...
define(['exports', './when-7ef6387a', './DeveloperError-101d8eb9', './Check-71521db1', './Ellipsoid-e5098033', './Transforms-0f2c35ae', './Cartesian2-ad22f1aa', './PrimitiveType-4c1d698a'], function (exports, when, DeveloperError, Check, Ellipsoid, Transforms, Cartesian2, PrimitiveType) { 'use strict';
    ...
    /**
            ...
            for ( var property in geometry.attributes) {
                if (geometry.attributes.hasOwnProperty(property) &&
                    ...
                    if ((numberOfVertices !== num) && (numberOfVertices !== -1)) {
                        throw new DeveloperError.DeveloperError('All attribute lists must have the same number of attributes.');
                    }
                    //>>includeEnd('debug');
        ...
        var rectangleCenterScratch = new Ellipsoid.Cartographic();
        var enuCenterScratch = new Ellipsoid.Cartesian3();
        var fixedFrameToEnuScratch = new Transforms.Matrix4();
        var boundingRectanglePointsCartographicScratch = [new Ellipsoid.Cartographic(), new Ellipsoid.Cartographic(), new Ellipsoid.Cartographic()];
        ...
        var pointEnuScratch = new Ellipsoid.Cartesian3();
        ...
        Geometry._textureCoordinateRotationPoints = function(positions, stRotation, ellipsoid, boundingRectangle) {
            ...
            var enuCenter = Ellipsoid.Cartographic.toCartesian(rectangleCenter, ellipsoid, enuCenterScratch);
            ...
            for (i = 0; i < 3; i++) {
                Ellipsoid.Cartographic.toCartesian(boundingPointsCarto[i], ellipsoid, posEnu);
            ...
            var rotation = Transforms.Quaternion.fromAxisAngle(Ellipsoid.Cartesian3.UNIT_Z, -stRotation, enuRotationScratch);
    ...
    /**
        ...
        function GeometryAttribute(options) {
            ...
            if (!when.defined(options.componentDatatype)) {
                throw new DeveloperError.DeveloperError('options.componentDatatype is required.');
            }
            if (!when.defined(options.componentsPerAttribute)) {
                throw new DeveloperError.DeveloperError('options.componentsPerAttribute is required.');
            }
            if (options.componentsPerAttribute < 1 || options.componentsPerAttribute > 4) {
                throw new DeveloperError.DeveloperError('options.componentsPerAttribute must be between 1 and 4.');
            }
            if (!when.defined(options.values)) {
                throw new DeveloperError.DeveloperError('options.values is required.');
            }
            //>>includeEnd('debug');
...
//# sourceMappingURL=GeometryAttribute-cb1e0b8f.js.map
