...
define(['exports', './when-7ef6387a', './Math-4ee1dfaf', './Ellipsoid-e5098033', './Transforms-0f2c35ae', './Cartesian2-ad22f1aa', './ComponentDatatype-f4fabeaa', './GeometryAttribute-cb1e0b8f', './PrimitiveType-4c1d698a', './GeometryAttributes-cb18da36', './GeometryPipeline-6475d5b1', './IndexDatatype-ff78eee3', './arrayRemoveDuplicates-d5806b46', './ArcType-2ee8dfbb', './EllipsoidRhumbLine-89ea2621', './PolygonPipeline-014b32c8'], function (exports, when, _Math, Ellipsoid, Transforms, Cartesian2, ComponentDatatype, GeometryAttribute, PrimitiveType, GeometryAttributes, GeometryPipeline, IndexDatatype, arrayRemoveDuplicates, ArcType, EllipsoidRhumbLine, PolygonPipeline) { 'use strict';
    ...
    /**
        ...
        PolygonGeometryLibrary.computeHierarchyPackedLength = function(polygonHierarchy) {
            ...
            while (stack.length > 0) {
                ...
                if (when.defined(positions)) {
                    numComponents += positions.length * Ellipsoid.Cartesian3.packedLength;
                }
            ...
            while (stack.length > 0) {
                ...
                if (when.defined(positions)) {
                    ...
                    for (var i = 0; i < positionsLength; ++i, startingIndex += 3) {
                        Ellipsoid.Cartesian3.pack(positions[i], array, startingIndex);
                    }
                }
        ...
        PolygonGeometryLibrary.unpackPolygonHierarchy = function(array, startingIndex) {
            ...
            for (var i = 0; i < positionsLength; ++i, startingIndex += Ellipsoid.Cartesian3.packedLength) {
                positions[i] = Ellipsoid.Cartesian3.unpack(array, startingIndex);
            }
        ...
        var distanceScratch = new Ellipsoid.Cartesian3();
        function getPointAtDistance(p0, p1, distance, length) {
            Ellipsoid.Cartesian3.subtract(p1, p0, distanceScratch);
            Ellipsoid.Cartesian3.multiplyByScalar(distanceScratch, distance / length, distanceScratch);
            Ellipsoid.Cartesian3.add(p0, distanceScratch, distanceScratch);
            return [distanceScratch.x, distanceScratch.y, distanceScratch.z];
        }

        PolygonGeometryLibrary.subdivideLineCount = function(p0, p1, minDistance) {
            var distance = Ellipsoid.Cartesian3.distance(p0, p1);
            var n = distance / minDistance;
            var countDivide = Math.max(0, Math.ceil(_Math.CesiumMath.log2(n)));
            return Math.pow(2, countDivide);
        };

        var scratchCartographic0 = new Ellipsoid.Cartographic();
        var scratchCartographic1 = new Ellipsoid.Cartographic();
        var scratchCartographic2 = new Ellipsoid.Cartographic();
        var scratchCartesian0 = new Ellipsoid.Cartesian3();
        ...
        PolygonGeometryLibrary.subdivideLine = function(p0, p1, minDistance, result) {
            ...
            var length = Ellipsoid.Cartesian3.distance(p0, p1);
        ...
        var scaleToGeodeticHeightN1 = new Ellipsoid.Cartesian3();
        var scaleToGeodeticHeightN2 = new Ellipsoid.Cartesian3();
        var scaleToGeodeticHeightP1 = new Ellipsoid.Cartesian3();
        var scaleToGeodeticHeightP2 = new Ellipsoid.Cartesian3();
        ...
        PolygonGeometryLibrary.scaleToGeodeticHeightExtruded = function(geometry, maxHeight, minHeight, ellipsoid, perPositionHeight) {
            ...
            if (when.defined(geometry) && when.defined(geometry.attributes) && when.defined(geometry.attributes.position)) {
                ...
                for ( var i = 0; i < length; i += 3) {
                    ...
                    n2 = Ellipsoid.Cartesian3.multiplyByScalar(n1, minHeight, n2);
                    n2 = Ellipsoid.Cartesian3.add(p2, n2, n2);
                    positions[i + length] = n2.x;
                    positions[i + 1 + length] = n2.y;
                    positions[i + 2 + length] = n2.z;

                    if (perPositionHeight) {
                        p2 = Ellipsoid.Cartesian3.clone(p, p2);
                    }
                    n2 = Ellipsoid.Cartesian3.multiplyByScalar(n1, maxHeight, n2);
                    n2 = Ellipsoid.Cartesian3.add(p2, n2, n2);
                    positions[i] = n2.x;
                    positions[i + 1] = n2.y;
                    positions[i + 2] = n2.z;
        ...
        PolygonGeometryLibrary.polygonOutlinesFromHierarchy = function(polygonHierarchy, scaleToEllipsoidSurface, ellipsoid) {
            ...
            while (queue.length !== 0) {
                ...
                outerRing = arrayRemoveDuplicates.arrayRemoveDuplicates(outerRing, Ellipsoid.Cartesian3.equalsEpsilon, true);
                ...
                for (i = 0; i < numChildren; i++) {
                    ...
                    holePositions = arrayRemoveDuplicates.arrayRemoveDuplicates(holePositions, Ellipsoid.Cartesian3.equalsEpsilon, true);
        ...
        var computeBoundingRectangleCartesian3 = new Ellipsoid.Cartesian3();
        ...
        PolygonGeometryLibrary.computeBoundingRectangle = function (planeNormal, projectPointTo2D, positions, angle, result) {
            ...
            for ( var i = 0; i < length; ++i) {
                var p = Ellipsoid.Cartesian3.clone(positions[i], computeBoundingRectangleCartesian3);
        ...
        PolygonGeometryLibrary.computeWallGeometry = function(positions, ellipsoid, granularity, perPositionHeight, arcType) {
            ...
            for (i = 0; i < length; i++) {
                ...
                p1 = Ellipsoid.Cartesian3.fromArray(edgePositions, UL * 3, p1Scratch);
                p2 = Ellipsoid.Cartesian3.fromArray(edgePositions, UR * 3, p2Scratch);
                if (Ellipsoid.Cartesian3.equalsEpsilon(p1, p2, _Math.CesiumMath.EPSILON10, _Math.CesiumMath.EPSILON10)) {
                    //skip corner
                    continue;
                }
...
//# sourceMappingURL=PolygonGeometryLibrary-395d93e1.js.map
