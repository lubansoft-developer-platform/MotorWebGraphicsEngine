...
define(['exports', './when-7ef6387a', './DeveloperError-101d8eb9', './Check-71521db1', './Math-4ee1dfaf', './Ellipsoid-e5098033', './Transforms-0f2c35ae', './ComponentDatatype-f4fabeaa', './GeometryAttribute-cb1e0b8f', './PrimitiveType-4c1d698a', './GeometryAttributes-cb18da36', './VertexFormat-64d8e6c6', './Plane-ed675142'], function (exports, when, DeveloperError, Check, _Math, Ellipsoid, Transforms, ComponentDatatype, GeometryAttribute, PrimitiveType, GeometryAttributes, VertexFormat, Plane) { 'use strict';
    ...
    /**
        ...
        var faces = [new Ellipsoid.Cartesian3(), new Ellipsoid.Cartesian3(), new Ellipsoid.Cartesian3()];
        Ellipsoid.Cartesian3.clone(Ellipsoid.Cartesian3.UNIT_X, faces[0]);
        Ellipsoid.Cartesian3.clone(Ellipsoid.Cartesian3.UNIT_Y, faces[1]);
        Ellipsoid.Cartesian3.clone(Ellipsoid.Cartesian3.UNIT_Z, faces[2]);

        var scratchPlaneCenter = new Ellipsoid.Cartesian3();
        var scratchPlaneNormal = new Ellipsoid.Cartesian3();
        var scratchPlane = new Plane.Plane(new Ellipsoid.Cartesian3(1.0, 0.0, 0.0), 0.0);
        ...
        CullingVolume.fromBoundingSphere = function(boundingSphere, result) {
            ...
            if (!when.defined(boundingSphere)) {
                throw new DeveloperError.DeveloperError('boundingSphere is required.');
            }
            //>>includeEnd('debug');
            ...
            for (var i = 0; i < length; ++i) {
                ...
                Ellipsoid.Cartesian3.multiplyByScalar(faceNormal, -radius, scratchPlaneCenter);
                Ellipsoid.Cartesian3.add(center, scratchPlaneCenter, scratchPlaneCenter);

                plane0.x = faceNormal.x;
                plane0.y = faceNormal.y;
                plane0.z = faceNormal.z;
                plane0.w = -Ellipsoid.Cartesian3.dot(faceNormal, scratchPlaneCenter);
                ...
                plane1.w = -Ellipsoid.Cartesian3.dot(Ellipsoid.Cartesian3.negate(faceNormal, scratchPlaneNormal), scratchPlaneCenter);
        ...
        CullingVolume.prototype.computeVisibility = function(boundingVolume) {
            ...
            if (!when.defined(boundingVolume)) {
                throw new DeveloperError.DeveloperError('boundingVolume is required.');
            }
            //>>includeEnd('debug');
        ...
        CullingVolume.prototype.computeVisibilityWithPlaneMask = function(boundingVolume, parentPlaneMask) {
            ...
            if (!when.defined(parentPlaneMask)) {
                throw new DeveloperError.DeveloperError('parentPlaneMask is required.');
            }
            //>>includeEnd('debug');
    ...
    /**
        ...
        function update(frustum) {
            ...
            if (!when.defined(frustum.right) || !when.defined(frustum.left) ||
                ...
                throw new DeveloperError.DeveloperError('right, left, top, bottom, near, or far parameters are not set.');
            }
            //>>includeEnd('debug');
            ...
            if (frustum.top !== frustum._top || frustum.bottom !== frustum._bottom ||
                ...
                if (frustum.left > frustum.right) {
                    throw new DeveloperError.DeveloperError('right must be greater than left.');
                }
                if (frustum.bottom > frustum.top) {
                    throw new DeveloperError.DeveloperError('top must be greater than bottom.');
                }
                if (frustum.near <= 0 || frustum.near > frustum.far) {
                    throw new DeveloperError.DeveloperError('near must be greater than zero and less than far.');
                }
                //>>includeEnd('debug');
        ...
        var getPlanesRight = new Ellipsoid.Cartesian3();
        var getPlanesNearCenter = new Ellipsoid.Cartesian3();
        var getPlanesPoint = new Ellipsoid.Cartesian3();
        var negateScratch = new Ellipsoid.Cartesian3();
        ...
        OrthographicOffCenterFrustum.prototype.computeCullingVolume = function(position, direction, up) {
            ...
            if (!when.defined(up)) {
                throw new DeveloperError.DeveloperError('up is required.');
            }
            //>>includeEnd('debug');
            ...
            var right = Ellipsoid.Cartesian3.cross(direction, up, getPlanesRight);
            Ellipsoid.Cartesian3.normalize(right, right);
            var nearCenter = getPlanesNearCenter;
            Ellipsoid.Cartesian3.multiplyByScalar(direction, n, nearCenter);
            Ellipsoid.Cartesian3.add(position, nearCenter, nearCenter);

            var point = getPlanesPoint;

            // Left plane
            Ellipsoid.Cartesian3.multiplyByScalar(right, l, point);
            Ellipsoid.Cartesian3.add(nearCenter, point, point);
            ...
            plane.y = right.y;
            plane.z = right.z;
            plane.w = -Ellipsoid.Cartesian3.dot(right, point);

            // Right plane
            Ellipsoid.Cartesian3.multiplyByScalar(right, r, point);
            Ellipsoid.Cartesian3.add(nearCenter, point, point);
            ...
            plane.y = -right.y;
            plane.z = -right.z;
            plane.w = -Ellipsoid.Cartesian3.dot(Ellipsoid.Cartesian3.negate(right, negateScratch), point);

            // Bottom plane
            Ellipsoid.Cartesian3.multiplyByScalar(up, b, point);
            Ellipsoid.Cartesian3.add(nearCenter, point, point);
            ...
            plane.y = up.y;
            plane.z = up.z;
            plane.w = -Ellipsoid.Cartesian3.dot(up, point);

            // Top plane
            Ellipsoid.Cartesian3.multiplyByScalar(up, t, point);
            Ellipsoid.Cartesian3.add(nearCenter, point, point);
            ...
            plane.y = -up.y;
            plane.z = -up.z;
            plane.w = -Ellipsoid.Cartesian3.dot(Ellipsoid.Cartesian3.negate(up, negateScratch), point);
            ...
            plane.y = direction.y;
            plane.z = direction.z;
            plane.w = -Ellipsoid.Cartesian3.dot(direction, nearCenter);

            // Far plane
            Ellipsoid.Cartesian3.multiplyByScalar(direction, f, point);
            Ellipsoid.Cartesian3.add(position, point, point);
            ...
            plane.y = -direction.y;
            plane.z = -direction.z;
            plane.w = -Ellipsoid.Cartesian3.dot(Ellipsoid.Cartesian3.negate(direction, negateScratch), point);
        ...
        OrthographicOffCenterFrustum.prototype.getPixelDimensions = function(drawingBufferWidth, drawingBufferHeight, distance, pixelRatio, result) {
            ...
            if (!when.defined(drawingBufferWidth) || !when.defined(drawingBufferHeight)) {
                throw new DeveloperError.DeveloperError('Both drawingBufferWidth and drawingBufferHeight are required.');
            }
            if (drawingBufferWidth <= 0) {
                throw new DeveloperError.DeveloperError('drawingBufferWidth must be greater than zero.');
            }
            if (drawingBufferHeight <= 0) {
                throw new DeveloperError.DeveloperError('drawingBufferHeight must be greater than zero.');
            }
            if (!when.defined(distance)) {
                throw new DeveloperError.DeveloperError('distance is required.');
            }
            if (!when.defined(pixelRatio)) {
                throw new DeveloperError.DeveloperError('pixelRatio is required.');
            }
            if (pixelRatio <= 0) {
                throw new DeveloperError.DeveloperError('pixelRatio must be greater than zero.');
            }
            if (!when.defined(result)) {
                throw new DeveloperError.DeveloperError('A result object is required.');
            }
            //>>includeEnd('debug');
    ...
    /**
        ...
        function update$1(frustum) {
            ...
            if (!when.defined(frustum.width) || !when.defined(frustum.aspectRatio) || !when.defined(frustum.near) || !when.defined(frustum.far)) {
                throw new DeveloperError.DeveloperError('width, aspectRatio, near, or far parameters are not set.');
            }
            //>>includeEnd('debug');
            ...
            if (frustum.width !== frustum._width || frustum.aspectRatio !== frustum._aspectRatio ||
                ...
                if (frustum.aspectRatio < 0) {
                    throw new DeveloperError.DeveloperError('aspectRatio must be positive.');
                }
                if (frustum.near < 0 || frustum.near > frustum.far) {
                    throw new DeveloperError.DeveloperError('near must be greater than zero and less than far.');
                }
    ...
    /**
        ...
        var getPlanesRight$1 = new Ellipsoid.Cartesian3();
        var getPlanesNearCenter$1 = new Ellipsoid.Cartesian3();
        var getPlanesFarCenter = new Ellipsoid.Cartesian3();
        var getPlanesNormal = new Ellipsoid.Cartesian3();
        /**
         * Creates a culling volume for this frustum.
         *
         * @param {Cartesian3} position The eye position.
        ...
        PerspectiveOffCenterFrustum.prototype.computeCullingVolume = function(position, direction, up) {
            ...
            var right = Ellipsoid.Cartesian3.cross(direction, up, getPlanesRight$1);
            ...
            Ellipsoid.Cartesian3.multiplyByScalar(direction, f, farCenter);
            Ellipsoid.Cartesian3.add(position, farCenter, farCenter);

            var normal = getPlanesNormal;

            //Left plane computation
            Ellipsoid.Cartesian3.multiplyByScalar(right, l, normal);
            Ellipsoid.Cartesian3.add(nearCenter, normal, normal);
            Ellipsoid.Cartesian3.subtract(normal, position, normal);
            Ellipsoid.Cartesian3.normalize(normal, normal);
            Ellipsoid.Cartesian3.cross(normal, up, normal);
            Ellipsoid.Cartesian3.normalize(normal, normal);
            ...
            plane.y = normal.y;
            plane.z = normal.z;
            plane.w = -Ellipsoid.Cartesian3.dot(normal, position);

            //Right plane computation
            Ellipsoid.Cartesian3.multiplyByScalar(right, r, normal);
            Ellipsoid.Cartesian3.add(nearCenter, normal, normal);
            Ellipsoid.Cartesian3.subtract(normal, position, normal);
            Ellipsoid.Cartesian3.cross(up, normal, normal);
            Ellipsoid.Cartesian3.normalize(normal, normal);
            ...
            Ellipsoid.Cartesian3.multiplyByScalar(up, b, normal);
            Ellipsoid.Cartesian3.add(nearCenter, normal, normal);
            Ellipsoid.Cartesian3.subtract(normal, position, normal);
            Ellipsoid.Cartesian3.cross(right, normal, normal);
            Ellipsoid.Cartesian3.normalize(normal, normal);
            ...
            Ellipsoid.Cartesian3.multiplyByScalar(up, t, normal);
            Ellipsoid.Cartesian3.add(nearCenter, normal, normal);
            Ellipsoid.Cartesian3.subtract(normal, position, normal);
            Ellipsoid.Cartesian3.cross(normal, right, normal);
            Ellipsoid.Cartesian3.normalize(normal, normal);
            ...
            Ellipsoid.Cartesian3.negate(direction, normal);
            ...
            plane.y = normal.y;
            plane.z = normal.z;
            plane.w = -Ellipsoid.Cartesian3.dot(normal, farCenter);
    ...
    /**
        ...
        function update$3(frustum) {
            ...
            if (!when.defined(frustum.fov) || !when.defined(frustum.aspectRatio) || !when.defined(frustum.near) || !when.defined(frustum.far)) {
                throw new DeveloperError.DeveloperError('fov, aspectRatio, near, or far parameters are not set.');
            }
            //>>includeEnd('debug');
            ...
            if (frustum.fov !== frustum._fov || frustum.aspectRatio !== frustum._aspectRatio ||
                ...
                if (frustum.fov < 0 || frustum.fov >= Math.PI) {
                    throw new DeveloperError.DeveloperError('fov must be in the range [0, PI).');
                }
    ...
    var PERSPECTIVE = 0;
        ...
        function FrustumGeometry(options) {
            ...
            this._origin = Ellipsoid.Cartesian3.clone(origin);
             ...
             * @type {Number}
             */
            this.packedLength = 2 + frustumPackedLength + Ellipsoid.Cartesian3.packedLength + Transforms.Quaternion.packedLength + VertexFormat.VertexFormat.packedLength;
        }
        ...
        FrustumGeometry.pack = function(value, array, startingIndex) {
            ...
            Ellipsoid.Cartesian3.pack(value._origin, array, startingIndex);
            startingIndex += Ellipsoid.Cartesian3.packedLength;
        ...
        var scratchPackorigin = new Ellipsoid.Cartesian3();
        ...
        FrustumGeometry.unpack = function(array, startingIndex, result) {
            ...
            var origin = Ellipsoid.Cartesian3.unpack(array, startingIndex, scratchPackorigin);
            startingIndex += Ellipsoid.Cartesian3.packedLength;
            ...
            result._origin = Ellipsoid.Cartesian3.clone(origin, result._origin);
        ...
        var scratchXDirection = new Ellipsoid.Cartesian3();
        var scratchYDirection = new Ellipsoid.Cartesian3();
        var scratchZDirection = new Ellipsoid.Cartesian3();
        var scratchNegativeX = new Ellipsoid.Cartesian3();
        var scratchNegativeY = new Ellipsoid.Cartesian3();
        var scratchNegativeZ = new Ellipsoid.Cartesian3();
        ...
        FrustumGeometry._computeNearFarPlanes = function(origin, orientation, frustumType, frustum, positions, xDirection, yDirection, zDirection) {
            ...
            Ellipsoid.Cartesian3.normalize(x, x);
            Ellipsoid.Cartesian3.normalize(y, y);
            Ellipsoid.Cartesian3.normalize(z, z);

            Ellipsoid.Cartesian3.negate(x, x);
            ...
            for (var i = 0; i < 2; ++i) {
                for (var j = 0; j < 4; ++j) {
                    ...
                    if (!when.defined(inverseViewProjection)) {
                        ...
                        var w = 1.0 / corner.w;
                        Ellipsoid.Cartesian3.multiplyByScalar(corner, w, corner);

                        Ellipsoid.Cartesian3.subtract(corner, origin, corner);
                        Ellipsoid.Cartesian3.normalize(corner, corner);

                        var fac = Ellipsoid.Cartesian3.dot(z, corner);
                        Ellipsoid.Cartesian3.multiplyByScalar(corner, frustumSplits[i] / fac, corner);
                        Ellipsoid.Cartesian3.add(corner, origin, corner);
                    }
        ...
        FrustumGeometry.createGeometry = function(frustumGeometry) {
            ...
            if (when.defined(vertexFormat.normal) || when.defined(vertexFormat.tangent) || when.defined(vertexFormat.bitangent) || when.defined(vertexFormat.st)) {
                ...
                var negativeX = Ellipsoid.Cartesian3.negate(x, scratchNegativeX);
                var negativeY = Ellipsoid.Cartesian3.negate(y, scratchNegativeY);
                var negativeZ = Ellipsoid.Cartesian3.negate(z, scratchNegativeZ);
...
//# sourceMappingURL=FrustumGeometry-ebb4348b.js.map
