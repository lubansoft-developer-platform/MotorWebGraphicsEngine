...
define(['./when-7ef6387a', './Check-ed6a1804', './Cartesian3-18c04df5', './Ellipsoid-f29f901d', './Transforms-6e3c043b', './Matrix4-c68aaa66', './RuntimeError-5b606d78', './Cartesian2-e5f465dc', './WebGLConstants-30fc6f5c', './ComponentDatatype-a863af81', './EnvironmentVariables-7e1444a0', './AttributeCompression-414035f7', './IntersectionTests-6415221a', './Plane-f22e7e98', './WebMercatorProjection-96eb07e4', './createTaskProcessorWorker', './EllipsoidTangentPlane-ea3c97b6', './OrientedBoundingBox-cf9a8b04', './TerrainEncoding-2f5d28ee'], function (when, Check, Cartesian3, Ellipsoid, Transforms, Matrix4, RuntimeError, Cartesian2, WebGLConstants, ComponentDatatype, EnvironmentVariables, AttributeCompression, IntersectionTests, Plane, WebMercatorProjection, createTaskProcessorWorker, EllipsoidTangentPlane, OrientedBoundingBox, TerrainEncoding) { 'use strict';
    ...
    var sizeOfUint16 = Uint16Array.BYTES_PER_ELEMENT;
        ...
        function createVerticesFromGoogleEarthEnterpriseBuffer(parameters, transferableObjects) {
            parameters.ellipsoid = Ellipsoid.Ellipsoid.clone(parameters.ellipsoid);
            ...
            for (quad = 0; quad < 4; ++quad) {
                ...
                for (var i = 0; i < numPoints; ++i) {
                    ...
                    if (indexOfEpsilon(quadBorderLongitudes, longitude) !== -1 ||
                        ...
                        var index = indexOfEpsilon(quadBorderPoints, scratchCartographic, Ellipsoid.Cartographic);
                        if (index === -1) {
                            quadBorderPoints.push(Ellipsoid.Cartographic.clone(scratchCartographic));
                    ...
                    if (Math.abs(longitude - geographicWest) < halfStepX) {
                        westBorder.push({
                            index : pointOffset,
                            cartographic : Ellipsoid.Cartographic.clone(scratchCartographic)
                        });
        ...
        function addSkirt(positions, heights, uvs, webMercatorTs, indices, skirtOptions,
            ...
            for (var j = 0; j < count; ++j) {
                ...
                Ellipsoid.Cartographic.fromRadians(longitude, latitude, height, scratchCartographic);
